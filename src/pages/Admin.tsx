...
import {
  Search, Plus, Users, Shield, Settings, Key, Bell, Database, Activity,
...
const mockUsers = [
  { id: "USR-001", name: "Raj Kumar", email: "raj.kumar@company.com", role: "Admin", department: "Operations", status: "Active", lastLogin: "2 hours ago", createdAt: "Jan 15, 2023" },
  { id: "USR-002", name: "Priya Singh", email: "priya.singh@company.com", role: "Operations Manager", department: "Operations", status: "Active", lastLogin: "5 hours ago", createdAt: "Mar 20, 2023" },
  { id: "USR-003", name: "Amit Patel", email: "amit.patel@company.com", role: "Field Technician", department: "Maintenance", status: "Active", lastLogin: "1 day ago", createdAt: "Jun 10, 2023" },
  { id: "USR-004", name: "Sunita Reddy", email: "sunita.reddy@company.com", role: "Finance Manager", department: "Finance", status: "Active", lastLogin: "3 hours ago", createdAt: "Feb 05, 2023" },
  { id: "USR-005", name: "Vikram Joshi", email: "vikram.joshi@company.com", role: "Viewer", department: "Management", status: "Inactive", lastLogin: "2 weeks ago", createdAt: "Aug 15, 2023" },
];
...
export default function Admin() {
  ...
  const handleResetPassword = (user: UserType) => { setSelectedUser(user); setShowResetPasswordDialog(true); };
  ...
  return (
    <div className="p-6 animate-fade-in">
      ...
      <Tabs defaultValue="users" className="space-y-6">
                ...
                <TableBody>
                  {filteredUsers.map((user) => (
                    <TableRow key={user.id} className="hover:bg-muted/50 border-border/50">
                      <TableCell><div className="flex items-center gap-3"><Avatar className="w-9 h-9"><AvatarFallback className="bg-primary/10 text-primary text-sm">{user.name.split(" ").map((n) => n[0]).join("")}</AvatarFallback></Avatar><div><p className="font-medium text-foreground">{user.name}</p><p className="text-xs text-muted-foreground">{user.email}</p></div></div></TableCell>
                      <TableCell><Badge variant="outline">{user.role}</Badge></TableCell>
                      <TableCell className="text-muted-foreground">{user.department}</TableCell>
                      <TableCell><Badge className={user.status === "Active" ? "bg-[hsl(var(--status-success)/0.15)] text-[hsl(var(--status-success))] border-0" : "bg-muted text-muted-foreground border-0"}>{user.status}</Badge></TableCell>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="bg-popover border-border">
                            ...
                            <DropdownMenuItem onClick={() => handleResetPassword(user)}><Key className="w-4 h-4 mr-2" />Reset Password</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleDeactivateUser(user)}>{user.status === "Active" ? <><Lock className="w-4 h-4 mr-2" />Deactivate</> : <><Unlock className="w-4 h-4 mr-2" />Activate</>}</DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem className="text-[hsl(var(--status-error))]" onClick={() => handleDeleteUser(user)}><Trash2 className="w-4 h-4 mr-2" />Delete User</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </TableCell>
                    </TableRow>
        ...
        <TabsContent value="roles">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {roles.map((role) => (<Card key={role.name} className="data-card"><CardContent className="p-5"><div className="flex items-start justify-between mb-3"><Badge className={`${role.color} border-0`}>{role.name}</Badge></div><p className="text-sm text-muted-foreground mb-4">{role.description}</p><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Users className="w-4 h-4" /><span>{role.users} users</span></div><Button variant="outline" size="sm"><Edit className="w-4 h-4" />Edit</Button></div></CardContent></Card>))}
        ...
        <TabsContent value="audit">
          <Card className="border-border/60">
            <CardHeader className="pb-3"><div className="flex items-center justify-between"><div><CardTitle className="text-lg">Audit Log</CardTitle><CardDescription>Track all system activities</CardDescription></div><div className="flex gap-2"><Button variant="outline" size="sm"><RefreshCw className="w-4 h-4" />Refresh</Button><Button variant="outline" size="sm"><Download className="w-4 h-4" />Export</Button></div></div></CardHeader>
            <CardContent className="p-0">
      ...
      <DeleteUserDialog open={showDeleteUserDialog} onOpenChange={setShowDeleteUserDialog} user={selectedUser} />
      <ResetPasswordDialog open={showResetPasswordDialog} onOpenChange={setShowResetPasswordDialog} user={selectedUser} />
