...
const EDITABLE_FIELDS = [
  ...
  { key: "financialStatus", label: "Financial Status", type: "select", options: ["NOT_BILLED", "BILLED", "PAID", "OVERDUE", "REFUNDED"] },
  { key: "contractEndDate", label: "Contract End Date", type: "date" },
  { key: "preferredAcMake", label: "Preferred AC Make", type: "text" },
  { key: "siteType", label: "Site Type", type: "select", options: ["Residential", "Commercial", "Industrial"] },
  { key: "regionType", label: "Region Type", type: "select", options: ["Urban", "Rural", "Metro"] },
  { key: "state", label: "State", type: "text" },
  { key: "clientGstin", label: "Client GSTIN", type: "text" },
  { key: "stabilizerNumber", label: "Stabilizer Number", type: "text" },
  { key: "stabilizerOrderDate", label: "Stabilizer Order Date", type: "date" },
  { key: "stabilizerDeliveryDate", label: "Stabilizer Delivery Date", type: "date" },
  { key: "acOrderedDate", label: "AC Ordered Date", type: "date" },
  { key: "acDeliveredDate", label: "AC Delivered Date", type: "date" },
];

export function EditSiteDialog({ open, onOpenChange, siteId, siteName, onSuccess }: EditSiteDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedFields.size === 0) { toast.error("Select at least one field to change"); return; }
    ...
    try {
      ...
      const changeRequest = JSON.stringify({ changes, changeReason, changeDescription });
      ...
      await api.put(`/sites/${siteId}/edit`, formData, {
      ...
      toast.error(err?.response?.data || "Failed to update site");
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {siteName} — Select fields to change and provide a reason.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g., Site closure, Contract update" value={changeReason} onChange={(e) => setChangeReason(e.target.value)} />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {evidencePhotos.map((f, i) => (
                <Badge key={i} variant="outline" className="gap-1 text-xs">
                  {f.name.slice(0, 20)}
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setEvidencePhotos((p) => p.filter((_, idx) => idx !== i))} />
                </Badge>
              ))}
              <label className="inline-flex items-center gap-1 px-2 py-1 rounded border border-dashed border-border text-xs text-muted-foreground cursor-pointer hover:border-primary/40">
                ...
                <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => {
                  if (e.target.files) setEvidencePhotos((p) => [...p, ...Array.from(e.target.files!)]);
                }} />
              </label>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {evidenceDocument ? (
              <Badge variant="outline" className="gap-1 text-xs">
                {evidenceDocument.name.slice(0, 30)}
                <X className="w-3 h-3 cursor-pointer" onClick={() => setEvidenceDocument(null)} />
              </Badge>
            ) : (
              <label className="inline-flex items-center gap-1 px-2 py-1 rounded border border-dashed border-border text-xs text-muted-foreground cursor-pointer hover:border-primary/40">
                <Upload className="w-3 h-3" /> Upload Document
                <input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={(e) => {
                  if (e.target.files?.[0]) setEvidenceDocument(e.target.files[0]);
                }} />
              </label>
            )}
          </div>
